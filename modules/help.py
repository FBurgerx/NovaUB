...
def _render_module_detail(client, module_name, module, meta, pref):
    ...
    module_line = (
        "<blockquote>"
        f"<tg-emoji emoji-id=5897962422169243693>👻</emoji> "
    ...
    if commands:
        ...
        for cmd in commands:
            ...
            line = f"<code>{_escape(pref + cmd)}</code> — {_escape(desc)}"
            lines.append(line)
        cmds_block = "\n".join(lines)
    else:
        cmds_block = "<i>Нет команд</i>"

    commands_block = f"<blockquote expandable>{cmds_block}</blockquote>"
    author_line = (
        "<blockquote>"
...
async def help_cmd(client, message, args):
    ...
    if args and args[0].lower() not in LIST_ALIASES:
        ...
        if not module_name:
            ...
            text = (
                "<blockquote><tg-emoji emoji-id=5778527486270770928>❌</emoji> <b>Модуль не найден</b></blockquote>"
                f"{hint}\n"
    ...
    def format_mods(mods_dict):
        ...
        for mod, cmds in sorted(mods_dict.items()):
            ...
            res += f"<tg-emoji emoji-id=5877468380125990242>➡️</emoji> <b>{mod}</b> (<code>{cmds_str}</code>)\n"
        return res.strip()

    text = f"<tg-emoji emoji-id=5897962422169243693>👻</emoji> <b>NovaUB Modules</b>\n\n"
    if sys_mods:
