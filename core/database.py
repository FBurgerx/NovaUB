...
# База данных должна лежать в корне проекта, а не в core/.
...
def _abs(path: str) -> str:
    """Все пути к БД — относительно корня проекта."""
...
async def init(db_path: str = DB_PATH) -> None:
    ...
    async with _lock:
        if _db is not None:
            return
        _db = await aiosqlite.connect(_abs(db_path))
...
async def _migrate_legacy_module_configs() -> None:
    ...
    for legacy_path in (LEGACY_DB_PATH, LEGACY_DB_PATH_2):
        if not os.path.exists(legacy_path):
            continue
        try:
            async with aiosqlite.connect(_abs(legacy_path)) as legacy:
